...
3. **`memory/` exception**: there the format is the auto-memory's. `MEMORY.md` uses `[Title](file.md)`, and cross-links use the full file basename `[[type_slug]]` (e.g. `[[feedback_estilo]]`). Obsidian resolves the wikilink against the filename, so the label has to match the file exactly, `type_` prefix included.
