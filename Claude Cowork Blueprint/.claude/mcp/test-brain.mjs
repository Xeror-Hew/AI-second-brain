...
;(async () => {
  ...
  send({ jsonrpc: '2.0', id: 10, method: 'tools/call', params: { name: 'brain_log_done', arguments: { text: 'built and tested brain MCP' } } })
  ...
  const logAfter = readFileSync(join(PB, 'roadmap', 'roadmap_log.md'), 'utf8')
  check(/\d{4}-\d{2}-\d{2}: built and tested brain MCP/.test(logAfter) && !/def5678/.test(logAfter), 'log_done appended dated line, no hash')
  ...
  rmSync(VAULT, { recursive: true, force: true })   // tear down the fixture so it never ships in the template
