...
mk(join(PB, '.brain.json'), JSON.stringify({
  ...
  names: { Vision: 'Visao', context: 'contexto', next_step: 'proximo_passo', plan: 'plano', roadmap: 'roteiro', work_map: 'mapa_trabalho', library: 'biblioteca', notes: 'notas' },
...
;(async () => {
  ...
  send({ jsonrpc: '2.0', id: 5, method: 'tools/call', params: { name: 'brain_log_done', arguments: { text: 'tarefa localizada concluida' } } })
  ...
  check(/tarefa localizada concluida/.test(readFileSync(join(PB, 'roteiro', 'roteiro_log.md'), 'utf8')), 'log_done appended to localized roteiro/roteiro_log.md')
