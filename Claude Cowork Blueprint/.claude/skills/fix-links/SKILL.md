...
when_to_use: ALWAYS when you create, rename, or remove a file in project_brain/ (memory, work_map fragment, library fragment, plan, doc), in the same pass as the change
---

When you create, rename, or remove a doc in `project_brain/`:

1. **Index**: add or update the pointer in the right index (`work_map_index`, `library_index`, `plan_index`, `roadmap_index`, or `MEMORY.md`) with a path-style wikilink (`[[project_brain/...]]`) plus a one-line description. The description is what lets the AI find it without opening everything.
