---
name: end
description: Safety-net sweep at session close: catch up the map, log, roadmap, next step, dead links, and any unfinished branch that per-task closes missed.
...
5. **Branch**: if the session's work sits on a branch that is ready and unmerged, finish it now (the integration options in `/done`: merge, PR, keep, or discard) and clean up the worktree.
