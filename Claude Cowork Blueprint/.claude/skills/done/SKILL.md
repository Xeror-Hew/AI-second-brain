...
1. Add one line to `project_brain/roadmap/roadmap_log.md`: `{{date}}: {{what got done}} ({{file touched}})`. Append only.
2. Remove the finished item from `project_brain/roadmap/roadmap.md`. The active list only holds open work.
3. **Map**: if the task changed the shape of the work (new deliverable, moved/renamed file, reorganized folders), update `project_brain/work_map/` now (skill `/map`) while it's fresh. A small change with no structural impact: skip.
