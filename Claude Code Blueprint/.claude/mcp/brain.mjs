...
function toolLogDone(text, hash) {
  if (!text) return 'error: text required'
  ...
  const line = `- ${date}: ${text.trim()}${hash ? ' (' + hash + ')' : ''}`
  content = content.replace(/\s*$/, '') + '\n' + line + '\n'
...
const PROMPTS = {
  start: {
    ...
    text: 'A task just finished. Call brain_log_done with what shipped (and the commit hash if any), prune the finished item from the roadmap, and call brain_set_next with the next concrete action. If the code structure changed, update the code map.',
  },
  set_next: {
...
const TOOLS = [
  ...
  },
  {
    ...
    inputSchema: { type: 'object', properties: { text: { type: 'string' }, hash: { type: 'string' } }, required: ['text'], additionalProperties: false },
  },
]
...
function callTool(name, args) {
  switch (name) {
    ...
    case 'brain_log_done': return textResult(toolLogDone(args.text, args.hash))
    default: return null
  }
}
